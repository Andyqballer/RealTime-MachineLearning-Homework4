...
from tensorflow.keras.layers import Input, Embedding, GRU, Dense, Dot
...
inputs = Input(shape=(max_eng_length,))
embedding = Embedding(eng_vocab_size, 256, mask_zero=True)(inputs)
gru = GRU(256, return_sequences=True)(embedding)

# Calculate attention scores
attention_scores = Dense(max_eng_length, activation='softmax')(gru)

# Apply attention weights to the GRU output
context = Dot(axes=1)([attention_scores, gru])

# Final output layer
output = Dense(fra_vocab_size, activation='softmax')(context)
...
print(f"French Translation (with Attention): {translated_sentence}")
